...
public abstract class StatementCreatorUtils {
	...
	static boolean shouldIgnoreGetParameterType = false;

	private static final Logger LOG = Logger.getLogger(StatementCreatorUtils.class.getName());
	...
	private static void setParameterValueInternal(PreparedStatement ps, int paramIndex, int sqlType,
		...
		if (inValue instanceof SqlParameterValue) {
			...
			if (LOG.isLoggable(Level.FINE)) {
				LOG.log(Level.FINE, "Overriding type info with runtime info from SqlParameterValue: column index " + paramIndex +
		...
		if (LOG.isLoggable(Level.FINE)) {
			LOG.log(Level.FINE, "Setting SQL statement parameter value: column index " + paramIndex +
		...
		if (sqlType == SqlTypeValue.TYPE_UNKNOWN || (sqlType == Types.OTHER && typeName == null)) {
			...
			if (!shouldIgnoreGetParameterType) {
				try {
					...
					if (LOG.isLoggable(Level.FINE)) {
						LOG.log(Level.FINE, "JDBC getParameterType call failed - using fallback method instead: " + ex);
					}
				}
			}
