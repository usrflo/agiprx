 ...
 * integration testing support provided in the {@code org.springframework.test}
 * package, shipped in {@code spring-test.jar}.
 *
 * @author Rod Johnson
...
public interface JdbcOperations {
	...
	/**
	 * Execute a query given static SQL, mapping each row to a result object via a
	 * RowMapper, and turning it into an iterable and closeable Stream.
	 * <p>
	 * Uses a JDBC Statement, not a PreparedStatement. If you want to execute a
	 * static query with a PreparedStatement, use the overloaded {@code query}
	 * method with {@code null} as argument array.
	...
	/**
	 * Execute a query given static SQL, mapping a single result row to a result
	 ...
	 *         SQL NULL
	 * @throws IncorrectResultSizeDataAccessException if the query does not return
	 *                                                exactly one row, or does not
	 *                                                return exactly one column in
	 *                                                that row
	 * @throws DataAccessException                    if there is any problem
	 *                                                executing the query
	...
	// <T> T queryForObject(String sql, Class<T> requiredType) throws
	// DataAccessException;
	 ...
	 * Uses a JDBC Statement, not a PreparedStatement. If you want to execute a
	 * static query with a PreparedStatement, use the overloaded
	 * {@link #queryForMap(String, Object...)} method with {@code null} as argument
	 * array.
	 * <p>
	 ...
	 *                    example, {@code Integer.class})
	 * @return a List of objects that match the specified element type
	 * @throws DataAccessException if there is any problem executing the query
	 * @see #queryForList(String, Class, Object...)
	 * @see SingleColumnRowMapper
	 */
	// <T> List<T> queryForList(String sql, Class<T> elementType) throws
	// DataAccessException;
	 ...
	 * list will be of the form returned by this interface's {@code queryForMap}
	 * methods.
	 * 
	 * @param sql the SQL query to execute
	 ...
	 * available at runtime: by default, Sun's
	 * {@code com.sun.rowset.CachedRowSetImpl} class is used, which is part of JDK
	 * 1.5+ and also available separately as part of Sun's JDBC RowSet
	 * Implementations download (rowset.jar).
	 * 
	 * @param sql the SQL query to execute
	 ...
	 * ResultSetExtractor.
	 * <p>
	 * A PreparedStatementCreator can either be implemented directly or configured
	 * through a PreparedStatementCreatorFactory.
	 * 
	 * @param psc a callback that creates a PreparedStatement given a Connection
	 ...
	 *            bind parameters. Even if there are no bind parameters, this
	 *            callback may be used to set the fetch size and other performance
	 *            options.
	 * @param rse a callback that will extract results
	 * @return an arbitrary result object, as returned by the ResultSetExtractor
	 * @throws DataAccessException if there is any problem
	 ...
	 *                  to contain no bind parameters. Even if there are no bind
	 *                  parameters, this callback may be used to set the fetch size
	 ...
	 * Query given SQL to create a prepared statement from SQL and a
	 * PreparedStatementSetter implementation that knows how to bind values to the
	 * query, mapping each row to a result object via a RowMapper, and turning it
	 * into an iterable and closeable Stream.
	 * 
	 * @param sql       the SQL query to execute
	 * @param pss       a callback that knows how to set values on the prepared
	 *                  statement. If this is {@code null}, the SQL will be assumed
	 ...
	 * @param args         arguments to bind to the query (leaving it to the
	 *                     PreparedStatement to guess the corresponding SQL type);
	 *                     may also contain {@link SqlParameterValue} objects which
	 *                     indicate not only the argument value but also the SQL
	...
	/**
	 * Issue a single SQL update operation (such as an insert, update or delete
	 * statement) using a PreparedStatementCreator to provide SQL and any required
	 * parameters.
	 * <p>
	 * A PreparedStatementCreator can either be implemented directly or configured
	 * through a PreparedStatementCreatorFactory.
	 ...
	 * activated extraction of generated keys (a JDBC 3.0 feature). This can either
	 * be done directly or through using a PreparedStatementCreatorFactory.
	 * 
	 * @param psc                a callback that provides SQL and any necessary
	 *                           parameters
	 ...
	 * required parameters.
	 * 
	 * @param csc                a callback that provides SQL and any necessary
	 *                           parameters
