...
import de.agitos.agiprx.db.util.NumberUtils;
...
public abstract class JdbcUtils {
	...
	public static void closeConnection(Connection con) {
		if (con != null) {
			try {
				...
				LOG.log(Level.FINE, "Could not close JDBC Connection", ex);
			} catch (Throwable ex) {
				// We don't trust the JDBC driver: It might throw RuntimeException or Error.
				LOG.log(Level.FINE, "Unexpected exception on closing JDBC Connection", ex);
			}
		}
	}
	...
	public static void closeStatement(Statement stmt) {
		if (stmt != null) {
			try {
				...
				LOG.log(Level.FINE, "Could not close JDBC Statement", ex);
			} catch (Throwable ex) {
				// We don't trust the JDBC driver: It might throw RuntimeException or Error.
				LOG.log(Level.FINE, "Unexpected exception on closing JDBC Statement", ex);
			}
		}
	}
	...
	public static void closeResultSet(ResultSet rs) {
		if (rs != null) {
			try {
				...
				LOG.log(Level.FINE, "Could not close JDBC ResultSet", ex);
			} catch (Throwable ex) {
				// We don't trust the JDBC driver: It might throw RuntimeException or Error.
				LOG.log(Level.FINE, "Unexpected exception on closing JDBC ResultSet", ex);
			}
		}
	}
	 ...
	 * @return the value object (possibly not of the specified required type, with
	 *         further conversion steps necessary)
	...
	public static Object getResultSetValue(ResultSet rs, int index, Class<?> requiredType) throws SQLException {
		...
		if (String.class == requiredType) {
			...
			try {
				...
				LOG.log(Level.FINE, "JDBC driver does not implement JDBC 4.1 'getObject(int, Class)' method", err);
			} catch (SQLFeatureNotSupportedException ex) {
				LOG.log(Level.FINE, "JDBC driver does not support JDBC 4.1 'getObject(int, Class)' method", ex);
			} catch (SQLException ex) {
				LOG.log(Level.FINE, "JDBC driver has limited support for JDBC 4.1 'getObject(int, Class)' method", ex);
			}
	 ...
	 * <p>
	 * Uses the {@code getObject(index)} method, but includes additional "hacks" to
	 * get around Oracle 10g returning a non-standard object for its TIMESTAMP
	 * datatype and a {@code java.sql.Date} for DATE columns leaving out the time
	 * portion: These columns will explicitly be extracted as standard
	 ...
	 * <p>
	 * This method will open a connection to the database and retrieve its
	 * meta-data. Since this method is called before the exception translation
	 * feature is configured for a DataSource, this method can not rely on
	 * SQLException translation itself.
	 * <p>
	 * Any exceptions will be wrapped in a MetaDataAccessException. This is a
	 * checked exception and any calling code should catch and handle this
	 * exception. You can just log the error and hope for the best, but there is
	 * probably a more serious error that will reappear when you try to access the
	 * database again.
	 * 
	...
	public static <T> T extractDatabaseMetaData(DataSource dataSource, DatabaseMetaDataCallback<T> action)
		...
		try {
			...
			try {
				...
				/*
				 * if (DataSourceUtils.isConnectionTransactional(con, dataSource)) { // Probably
				 * a closed thread-bound Connection - retry against fresh Connection
				 * DataSourceUtils.releaseConnection(con); con = null; logger.
				 * debug("Failed to obtain DatabaseMetaData from transactional Connection - " +
				 * "retrying against fresh Connection", ex); con = dataSource.getConnection();
				 * metaData = con.getMetaData(); } else { throw ex; }
				 */
				throw ex;
			}
			if (metaData == null) {
	...
	/**
	 * Call the specified method on DatabaseMetaData for the given DataSource, and
	 * extract the invocation result.
	 * 
	 * @param dataSource         the DataSource to extract meta-data for
	 ...
	 *             with a lambda expression or method reference and a generically
	 *             typed result
	 */
	@Deprecated
		...
		try {
			...
			if (dbmd != null) {
				if (dbmd.supportsBatchUpdates()) {
					LOG.log(Level.FINE, "JDBC driver supports batch updates");
					return true;
				} else {
					LOG.log(Level.FINE, "JDBC driver does not support batch updates");
				}
			}
		} catch (SQLException ex) {
			LOG.log(Level.FINE, "JDBC driver 'supportsBatchUpdates' method threw exception", ex);
		}
		return false;
	}
