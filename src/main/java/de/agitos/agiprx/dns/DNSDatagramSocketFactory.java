...
class DNSDatagramSocketFactory {
	...
	/**
	 * The dynamic allocation port range (aka ephemeral ports), as configured on the
	 * system. Use nested class for lazy evaluation.
	 */
	static final class EphemeralPortRange {
		private EphemeralPortRange() {
		}

		// lower max and upper min from sun.net.PortConfig
		static final int LOWER = 49152;
		static final int UPPER = 61000;
	...
	/**
	 * Opens a datagram socket listening to the wildcard address on a random port.
	 * If the underlying OS supports UDP port randomization out of the box (if
	 * binding a socket to port 0 binds it to a random port) then the underlying OS
	 * implementation is used. Otherwise, this method will allocate and bind a
	 * socket on a randomly selected ephemeral port in the dynamic range.
	 * 
	...
	synchronized boolean isUsingNativePortRandomization() {
		return unsuitablePortCount <= thresholdCount && suitablePortCount > thresholdCount;
	}
	...
	synchronized boolean isUndecided() {
		return !isUsingJavaPortRandomization() && !isUsingNativePortRandomization();
	}

	private DatagramSocket openRandom() {
		int maxtries = MAX_RANDOM_TRIES;
		while (maxtries-- > 0) {
			int port = EphemeralPortRange.LOWER + random.nextInt(EphemeralPortRange.RANGE);
			try {
				if (family != null) {
					DatagramChannel c = DatagramChannel.open(family);
					try {
