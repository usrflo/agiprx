...
public class DnsClient {
	...
	/*
	 * Each server is of the form "server[:port]". IPv6 literal host names include
	 * delimiting brackets. "timeout" is the initial timeout interval (in ms) for
	 * UDP queries, and "retries" gives the number of retries per server.
	 */
	public DnsClient(String[] servers, int timeout, int retries) throws NamingException {
		...
		reqs = Collections.synchronizedMap(new HashMap<Integer, ResourceRecord>());
	...
	/*
	 * If recursion is true, recursion is requested on the query. If auth is true,
	 * only authoritative responses are accepted; other responses throw
	...
	ResourceRecords query(DnsName fqdn, int qclass, int qtype, boolean recursion, boolean auth) throws NamingException {
		...
		do {
			...
			collision = reqs.putIfAbsent(xid,
					new ResourceRecord(pkt.getData(), pkt.length(), Header.HEADER_SIZE, true, false));
	...
	/**
	 * Tries to retrieve a UDP packet matching the given xid received within the
	 * timeout. If a packet with different xid is received, the received packet is
	 * enqueued with the corresponding xid in 'resps'.
	 */
	private byte[] doUdpQuery(Packet pkt, InetAddress server, int port, int retry, int xid)
	...
	/*
	 * Checks the header of an incoming DNS response. Returns true if it matches the
	 * given xid and throws a naming exception, if appropriate, based on the
	 * response code.
	 *
	 * Also checks that the domain name, type and class in the response match those
	...
	private boolean isMatchResponse(byte[] pkt, int xid) throws NamingException {
		...
		if (hdr.xid == xid) {
			...
			if (!hdr.query && hdr.numQuestions == 1) {
				...
				ResourceRecord rr = new ResourceRecord(pkt, pkt.length, Header.HEADER_SIZE, true, false);
				...
				if ((qtype == ResourceRecord.QTYPE_STAR || qtype == rr.getType())
						&& (qclass == ResourceRecord.QCLASS_STAR || qclass == rr.getRrclass())
						&& qname.equals(rr.getName())) {

					if (debug) {
						dprint("MATCH NAME:" + qname + " QTYPE:" + qtype + " QCLASS:" + qclass);
					}
					...
					if (debug) {
						dprint("NO-MATCH NAME:" + qname + " QTYPE:" + qtype + " QCLASS:" + qclass);
					}
				}
			}
			return false;
