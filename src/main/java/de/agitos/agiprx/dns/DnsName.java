...
/**
 * {@code DnsName} implements compound names for DNS as specified by RFCs 1034
 * and 1035, and as updated and clarified by RFCs 1123 and 2181.
 *
 * <p>
 * The labels in a domain name correspond to JNDI atomic names. Each label must
 * be less than 64 octets in length, and only the optional root label at the end
 * of the name may be 0 octets long. The sum of the lengths of all labels in a
 * name, plus the number of non-root labels plus 1, must be less than 256. The
 * textual representation of a domain name consists of the labels, escaped as
 * needed, dot-separated, and ordered right-to-left.
 *
 * <p>
 * A label consists of a sequence of octets, each of which may have any value
 * from 0 to 255.
 *
 * <p>
 * <em>Host names</em> are a subset of domain names. Their labels contain only
 * ASCII letters, digits, and hyphens, and none may begin or end with a hyphen.
 * While names not conforming to these rules may be valid domain names, they
 * will not be usable by a number of DNS applications, and should in most cases
 * be avoided.
 *
 * <p>
 * DNS does not specify an encoding (such as UTF-8) to use for octets with
 * non-ASCII values. As of this writing there is some work going on in this
 * area, but it is not yet finalized. {@code DnsName} currently converts any
 * non-ASCII octets into characters using ISO-LATIN-1 encoding, in effect taking
 * the value of each octet and storing it directly into the low-order byte of a
 * Java character and <i>vice versa</i>. As a consequence, no character in a DNS
 * name will ever have a non-zero high-order byte. When the work on
 * internationalizing domain names has stabilized (see for example
 * <i>draft-ietf-idn-idna-10.txt</i>), {@code DnsName} may be updated to conform
 * to that work.
 *
 * <p>
 * Backslash ({@code \}) is used as the escape character in the textual
 * representation of a domain name. The character sequence `{@code \DDD}', where
 * {@code DDD} is a 3-digit decimal number (with leading zeros if needed),
 * represents the octet whose value is {@code DDD}. The character sequence
 * `{@code \C}', where {@code C} is a character other than {@code '0'} through
 * {@code '9'}, represents the octet whose value is that of {@code C} (again
 * using ISO-LATIN-1 encoding); this is particularly useful for escaping
 * {@code '.'} or backslash itself. Backslash is otherwise not allowed in a
 * domain name. Note that escape characters are interpreted when a name is
 * parsed. So, for example, the character sequences `{@code S}', `{@code \S}',
 * and `{@code \083}' each represent the same one-octet name. The
 * {@code toString()} method does not generally insert escape sequences except
 * where necessary. If, however, the {@code DnsName} was constructed using
 * unneeded escapes, those escapes may appear in the {@code toString} result.
 *
 * <p>
 * Atomic names passed as parameters to methods of {@code DnsName}, and those
 * returned by them, are unescaped. So, for example,
 * <code>(new&nbsp;DnsName()).add("a.b")</code> creates an object representing
 * the one-label domain name {@code a\.b}, and calling {@code get(0)} on this
 * object returns {@code "a.b"}.
 *
 * <p>
 * While DNS names are case-preserving, comparisons between them are
 * case-insensitive. When comparing names containing non-ASCII octets,
 * {@code DnsName} uses case-insensitive comparison between pairs of ASCII
 * values, and exact binary comparison otherwise.
 * 
 * <p>
 * A {@code DnsName} instance is not synchronized against concurrent access by
 * multiple threads.
 *
...
public final class DnsName implements Name {
	 ...
	 * @throws InvalidNameException if {@code name} does not conform to DNS syntax.
	 */
	...
	/*
	 * Returns a new DnsName with its name components initialized to the components
	 * of "n" in the range [beg,end). Indexing is as for the Name interface, with 0
	...
	/*
	 * Helper method for public comparison methods. Lexicographically compares
	 * components of this name in the range [beg,end) with all components of "n".
	 * Indexing is as for the Name interface, with 0 being the most significant.
	 * Returns negative, zero, or positive as these name components are less than,
	 * equal to, or greater than those of "n".
	 */
	...
	/*
	 * Returns a key suitable for hashing the label at index i. Indexing is as for
	 * the Name interface, with 0 being the most significant.
	 */
	String getKey(int i) {
	...
	/*
	 * Returns (as a char) the octet indicated by the escape sequence at a given
	 * position within a domain name.
	 * 
	...
	/*
	 * Compares two labels, ignoring case for ASCII values. Returns negative, zero,
	 * or positive as the first label is less than, equal to, or greater than the
	 * second. See keyForLabel().
	 */
	...
	/*
	 * Returns a key suitable for hashing a label. Two labels map to the same key
	 * iff they are equal, taking possible case-folding into account. See
	 * compareLabels().
	 */
	...
	/**
	 * Serializes only the domain name string, for compactness and to avoid any
